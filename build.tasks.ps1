properties {
  ...
  $sln_file = "Bankleitzahlen-Tool.sln"
...
task Build -depends Clean, RestoreNuGetPackages {
  if (Test-Path Env:\APPVEYOR_BUILD_VERSION) {
    exec { & $msbuild_exe $sln_file /Logger:"C:\Program Files\AppVeyor\BuildAgent\Appveyor.MSBuildLogger.dll" }
  } else {
...
task Test -depends FindTestAssemblies -ContinueOnError {
  $appveyor_logger_vstest_console = ""
  if (Test-Path Env:\APPVEYOR_BUILD_VERSION) {
    $appveyor_logger_vstest_console = "/Logger:Appveyor"
  }
...
task ResolveCoveralls {
  $script:coveralls_exe = Resolve-Path "packages\coveralls.net.*\tools\csmacnz.Coveralls.exe"
}
